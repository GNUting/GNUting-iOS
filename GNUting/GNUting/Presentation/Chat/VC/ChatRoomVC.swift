...
extension ChatRoomVC{
    private func setAddSubViews() {
        view.addSubViews([subTitleLabel,borderView1,chatRoomTableView,borderView2,sendStackView,testView,opaqueView])
...
extension ChatRoomVC {
    ...
    @objc private func tapSettingButton(_ sender: UIButton){
        ...
        testView.transform = CGAffineTransform(translationX: 0, y: 0)
    ...
    @objc func handleCardPan(recognizer:UIPanGestureRecognizer) {
        let newLocation = recognizer.location(in: testView)
        
        switch recognizer.state{
          
        case .began:   // Pan 시작
            print("began")
            originalPostion = recognizer.location(in: testView)
        ...
        case .changed:         //Pan 하는중
            ...
            UIView.animate(withDuration: 0.5, delay: 0.0) {
                self.testView.transform = CGAffineTransform(translationX: xOffset, y: 0)
            }
            recognizer.setTranslation(CGPoint.zero, in: testView)
        case .ended: //Pan 종료
            let translation = CGAffineTransform(translationX: view.frame.width, y: 0)
            testView.transform = translation
